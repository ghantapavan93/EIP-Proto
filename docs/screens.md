...
| `/governance` | Separation of duties: the action × role matrix for everyone; for admins, the access review (accounts, default passwords, refusals), audit checkpoints with receipts anyone can verify, and adopting the rule corpus. Other roles see what each panel is and why the server refuses it to them |
